...
- **精选文章**：保留研究方向 AI 筛选、表格、多选、重新分析、正文采集和笔记保存。此页不显示个性化推荐面板。旧文章按已有研究方向匹配记录识别为精选。在此页点击“更新订阅”只执行精选所需的 RSS 抓取与 AI 筛选，不保存未入选文章，也不更新探索推荐。
- **探索模式**：包含文献阅读、订阅管理、兴趣分析三个子页。在此页点击“更新订阅”（或订阅管理中的“更新所有订阅”）只抓取并保存探索文章，不执行精选研究方向分析或推荐训练。探索的五个篮子排除精选文章。
...
- 精选页的四个排序按钮支持重复点击反转：切换到标题／期刊时默认字母正序，更新时间默认最新优先，相关度默认高优先；再点当前按钮反转，再点恢复。切换其他按钮会使用该按钮的默认方向。箭头显示当前方向，选择会保存；探索模式排序不受影响。
...
保存文献后，在 Obsidian 文件列表中右键该文献文件夹或文献笔记，选择 **导入到Zotero**。弹窗显示本地文件清单，并可选择 Zotero 文库／分类（含子分类）。请先启动 Zotero，并启用“设置 → 高级 → 允许此计算机上的其他应用程序与 Zotero 通信”。本功能使用 Zotero 9 本地连接器，无需云端 API Key；只向本机 `127.0.0.1:23119` 发送数据。

- 以笔记属性和已保存 RSS 信息创建一个文献条目，将同目录的 PDF、补充材料、同行评审文件复制为该条目的附件；也读取以 supplementary／peer-review／补充／同行评审命名的附件子目录。默认排除 `.bib`、`.md` 文件（不区分大小写，附件子目录也适用），不加入清单或上传队列；跳过 MinerU、Audio Tutor 等派生目录。只导入已经下载到本地的文件。
- 条目标题、作者、期刊、日期和 DOI 优先取自笔记属性；可在导入前修改笔记属性。Markdown 笔记仍用于识别文献和读取元数据，但不作为附件导入。仅有 `.bib`、`.md` 文件时导入按钮不可用。
- 文件夹需能唯一识别出插件保存的文献笔记（保存记录或 `ai-rss-reader` 标签）；多个文献请分别保存在各自文件夹，避免附件混入同一 Zotero 条目。分类只列出允许保存附件的目标。
- 附件失败会逐个列出，同一插件会话内点击“导入 / 补传未完成文件”只补传未成功的文件。插件重新加载或 Zotero 重启后不保证去重；再次导入前请检查 Zotero。超时／中断可能发生在实际写入之后，也应先检查是否已有附件。创建条目结果不确定时会阻止自动重建，以免重复。
- 导入后附件由 Zotero 管理，Obsidian 原文件保留。不会同步后续编辑；Zotero 自身的同步设置仍按其原有配置工作。

### AI 研究方向筛选

- 创建多个中英文研究方向并独立启用或停用
- 修改研究方向名称、描述、启用状态或数量后，下一次“更新订阅”会按新方向重新分析最近从 RSS 获取的文章。回溯范围可在设置中调整，默认近 1 天，`0` 表示关闭。强制重分析不受旧分析结果和关键词预筛影响；仅在整次分析成功后记录新方向版本，失败后再次更新仍会继续重试。手动导入的文章不在此范围内。
- 可选关键词预筛，减少模型调用量
- 一次请求判断文章与多个方向的相关性，并保存中文推荐理由
- 模型返回的 JSON 格式错误或被截断时，自动先拆成单个研究方向，再将论文批次对半拆小；单篇／单方向最多额外重试一次。补试缺失项时也应用此恢复逻辑，不猜测截断内容。持续失败会明确提示检查或更换模型；网络、鉴权及限流错误不会触发格式重试。拆分会增加模型请求次数。
- 模型漏掉部分论文／方向组合时，先按方向补试缺失项，再对残留缺项逐一进行一次单篇／单方向补试；仍失败会显示错误，不把缺失结果当作“不相关”。更新订阅会分批保存完整评估的文章；最终仍有缺项时，同批中其他完整评估也会保存。再次更新只重试没有完整评估的文章（仍应用关键词预筛），避免反复消耗已完成文章的分析请求。本地关键词评分完成不代表 AI 研究方向分析成功。
...
- **研究方向：**添加、编辑、删除、启用或停用方向，以及方向变更后的 RSS 文章重分析天数（默认 1 天）
